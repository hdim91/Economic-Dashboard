...
def build_dashboard_data() -> dict:
    ...
    # ── 4. 금융 핵심 지표 (finance_stats — pipeline 실행 후 채워짐) ────
    def _fin_latest(variable_name: str):
        """finance_raw_dedup에서 특정 variable_name의 최신값을 반환. 데이터 없으면 None."""
        try:
            fds = f"`{GCP_PROJECT}.{FINANCE_DATASET}.finance_raw_dedup`"
    ...
    fin_kr_base_rate  = _fin_latest("kr_base_rate")
    fin_kr_cpi        = _fin_latest("kr_cpi")
    # KOSIS 주택가격: adjustment_type 접두어 포함 — ECOS/KOSIS 중 최신값 사용
    fin_kr_house      = _fin_latest("none_kr_house_price_sale")
    fin_kr_usd        = _fin_latest("kr_usd_rate")      # ECOS 환율 (monthly avg)
    fin_us_fed        = _fin_latest("us_fed_funds_rate")

    def _fin_kpi(latest):
        return {"latest": latest} if latest else {"latest": None}
    ...
    return {
        ...
        "indicators": {
            ...
            # 금융 지표 (JS key → BQ variable_name 매핑)
            "kr_base_rate":  {**_fin_kpi(fin_kr_base_rate),  "label": "한국 기준금리",       "unit": "%"},
            "kr_cpi":        {**_fin_kpi(fin_kr_cpi),        "label": "소비자물가지수 (CPI)", "unit": "지수"},
            "kr_house_price":{**_fin_kpi(fin_kr_house),      "label": "주택매매가격지수",     "unit": "지수"},
            "kr_usd_krw":    {**_fin_kpi(fin_kr_usd),        "label": "원/달러 환율",         "unit": "원"},
            "us_fed_rate":   {**_fin_kpi(fin_us_fed),        "label": "미국 연방기금금리",    "unit": "%"},
        },
    }
